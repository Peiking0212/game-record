...
function AchievementForm({
  ...
  return (
    <form className="space-y-6" onSubmit={onSubmit}>
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">�ɾ�����</label>
        <input
          type="text"
          required
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">������Ϸ</label>
        <select
          required
          className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          value={values.gameId}
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">�ɾ�ͼ��</label>
        <select
          required
          className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          value={values.icon}
      ...
      {showScreenshotField && (
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">�ɾͽ�ͼ</label>
          <label className="upload-area block cursor-pointer">
            ...
            <p className="text-gray-600 mb-2 text-center">����ϴ���ͼ��JPG/PNG��</p>
          </label>
          {values.screenshot ? (
...
export function AchievementsClient() {
  ...
  const onAddSubmit = (e: React.FormEvent) => {
    ...
    if (!persist([...items, nextItem], "�ɾ������")) return;
    setAddOpen(false);
    setAddForm({
      title: "",
      description: "",
  ...
  const onEditSubmit = (e: React.FormEvent) => {
    ...
    if (!persist(next, "�ɾ���Ϣ�Ѹ���")) return;
    setEditItem(null);
  };

  const onDelete = (id: number | string) => {
    if (typeof window !== 'undefined' && !window.confirm("ȷ��Ҫɾ������ɾ��𣿴˲������ɳ�����")) return;
  ...
  return (
    <>
      ...
      <section className="py-16 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="max-w-6xl mx-auto mb-12 grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">����Ϸɸѡ</label>
              <select
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                value={gameFilter}
                onChange={(e) => setGameFilter(e.target.value)}
              >
                <option value="all">ȫ����Ϸ</option>
                {games.map((g) => (
                  <option key={String(g.id)} value={String(g.id)}>
                    {g.name}
                  </option>
          ...
          <div className="max-w-6xl mx-auto">
            {filtered.length === 0 ? (
              <div className="text-center py-16">
                ...
                <h3 className="text-2xl font-bold text-gray-700 mb-4">��û�гɾ�</h3>
                <p className="text-gray-600 mb-8">���������³ɾ͡���ť��ʼ��¼�����Ϸ�ɾ�</p>
                <button className="btn-primary" onClick={() => setAddOpen(true)}>
                  <Plus className="w-5 h-5 inline mr-2" />
                  ����³ɾ�
                </button>
                ...
                {filtered.map((achievement) => (
                  ...
                  >
                    <div className="p-6 bg-white rounded-lg shadow-lg border border-gray-100 h-full">
                      ...
                      <div className="text-sm text-gray-500">���ʱ��: {formatDateISO(achievement.date)}</div>
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold text-center mb-12 text-gray-800">�ɾ�ʱ����</h2>
      ...
      <Modal open={addOpen} onClose={() => setAddOpen(false)} title="����³ɾ�">
        <AchievementForm
          values={addForm}
      ...
      <Modal open={!!editItem} onClose={() => setEditItem(null)} title="�༭�ɾ�">
        <AchievementForm
          values={editForm}
      ...
      >
        {detailItem ? (
          <div className="space-y-6">
            ...
            <div>
              <h4 className="font-semibold text-gray-800 mb-2">�ɾ�����</h4>
            ...
            {detailItem.screenshot ? (
              <div>
                <h4 className="font-semibold text-gray-800 mb-2">�ɾͽ�ͼ</h4>
                {/* eslint-disable-next-line @next/next/no-img-element */}
