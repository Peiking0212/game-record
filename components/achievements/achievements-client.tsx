...
function AchievementForm({
  ...
  return (
    <form className="space-y-6" onSubmit={onSubmit}>
      <div>
        <label className="block text-sm font-medium mb-2" style={{ color: "var(--text-dark)" }}>成就名称</label>
        <input
          type="text"
          required
      ...
      <div>
        <label className="block text-sm font-medium mb-2" style={{ color: "var(--text-dark)" }}>成就描述</label>
        <textarea
          required
          rows={3}
          className="w-full px-4 py-2 border rounded-lg"
      ...
      <div>
        <label className="block text-sm font-medium mb-2" style={{ color: "var(--text-dark)" }}>所属游戏</label>
        <select
          required
          className="w-full px-4 py-2 border rounded-lg"
          style={{
      ...
      <div>
        <label className="block text-sm font-medium mb-2" style={{ color: "var(--text-dark)" }}>获得日期</label>
        <input
          required
          type="date"
          className="w-full px-4 py-2 border rounded-lg"
      ...
      <div>
        <label className="block text-sm font-medium mb-2" style={{ color: "var(--text-dark)" }}>成就图标</label>
        <select
          required
          className="w-full px-4 py-2 border rounded-lg"
          style={{
      ...
      {showScreenshotField && (
        <div>
          <label className="block text-sm font-medium mb-2" style={{ color: "var(--text-dark)" }}>成就截图</label>
          <label className="upload-area block cursor-pointer">
            ...
            <p className="mb-2 text-center" style={{ color: "var(--text-gray)" }}>点击上传截图（JPG/PNG）</p>
          </label>
          {values.screenshot ? (
...
export function AchievementsClient() {
  ...
  const onAddSubmit = (e: React.FormEvent) => {
    ...
    if (!persist([...items, nextItem], "成就已添加")) return;
    setAddOpen(false);
    setAddForm({
      title: "",
      description: "",
  ...
  const onEditSubmit = (e: React.FormEvent) => {
    ...
    if (!persist(next, "成就信息已更新")) return;
    setEditItem(null);
  };
  ...
  return (
    <>
      <PageHero
        ...
        title="成就系统"
        description="奖杯柜：解锁记录与获得日期写入总览，和主库成就读数同步。"
      >
      ...
      <PageSection alt>
          <div className="max-w-6xl mx-auto mb-12 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">搜索成就</label>
              <div className="relative">
                <input
                  type="text"
                  className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  placeholder="输入成就名称或游戏..."
                  value={search}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">按游戏筛选</label>
              <select
          ...
          <div className="max-w-6xl mx-auto">
            {filtered.length === 0 ? (
              <div className="text-center py-16 game-surface max-w-lg mx-auto px-8">
                ...
                <h3 className="text-2xl font-bold mb-4" style={{ color: "var(--text-dark)" }}>奖杯柜为空</h3>
                <p className="mb-8" style={{ color: "var(--text-gray)" }}>添加第一条解锁记录，总览会随之更新</p>
                ...
                {filtered.map((achievement) => (
                  ...
                  >
                    <div className="p-6 bg-white rounded-lg shadow-lg border border-gray-100 h-full">
                      ...
                      <div className="text-sm text-gray-500">获得时间: {formatDateISO(achievement.date)}</div>
      ...
      <Modal open={addOpen} onClose={() => setAddOpen(false)} title="添加新成就">
        <AchievementForm
          values={addForm}
      ...
      >
        {detailItem ? (
          <div className="space-y-6">
            ...
            <div>
              <h4 className="font-semibold text-gray-800 mb-2">成就描述</h4>
            ...
            {detailItem.screenshot ? (
              <div>
                <h4 className="font-semibold text-gray-800 mb-2">成就截图</h4>
                {/* eslint-disable-next-line @next/next/no-img-element */}
