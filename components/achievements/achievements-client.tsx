...
function AchievementForm({
  ...
  return (
    <form className="space-y-6" onSubmit={onSubmit}>
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">鎴愬氨鍚嶇О</label>
        <input
          type="text"
          required
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">鎴愬氨鎻忚堪</label>
        <textarea
          required
          rows={3}
          className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">鎵€灞炴父鎴?/label>
        <select
          required
          className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          value={values.gameId}
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">鑾峰緱鏃ユ湡</label>
        <input
          required
          type="date"
          className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">鎴愬氨鍥炬爣</label>
        <select
          required
          className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          value={values.icon}
      ...
      {showScreenshotField && (
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">鎴愬氨鎴浘</label>
          <label className="upload-area block cursor-pointer">
            ...
            <p className="text-gray-600 mb-2 text-center">鐐瑰嚮涓婁紶鎴浘锛圝PG/PNG锛?/p>
          </label>
          {values.screenshot ? (
...
export function AchievementsClient() {
  ...
  const onAddSubmit = (e: React.FormEvent) => {
    ...
    if (!persist([...items, nextItem], "鎴愬氨宸叉坊鍔?)) return;
    setAddOpen(false);
    setAddForm({
      title: "",
      description: "",
  ...
  const onEditSubmit = (e: React.FormEvent) => {
    ...
    if (!persist(next, "鎴愬氨淇℃伅宸叉洿鏂?)) return;
    setEditItem(null);
  };

  const onDelete = (id: number | string) => {
    if (!window.confirm("纭畾瑕佸垹闄よ繖涓垚灏卞悧锛熸鎿嶄綔涓嶅彲鎾ら攢銆?)) return;
    const next = items.filter((it) => String(it.id) !== String(id));
    persist(next, "鎴愬氨宸插垹闄?);
  ...
  return (
    <>
      ...
      <section className="py-16 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="max-w-6xl mx-auto mb-12 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">鎼滅储鎴愬氨</label>
              <div className="relative">
                <input
                  type="text"
                  className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  placeholder="杈撳叆鎴愬氨鍚嶇О鎴栨父鎴?.."
                  value={search}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">鎸夋父鎴忕瓫閫?/label>
              <select
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                value={gameFilter}
          ...
          <div className="max-w-6xl mx-auto">
            {filtered.length === 0 ? (
              <div className="text-center py-16">
                ...
                <h3 className="text-2xl font-bold text-gray-700 mb-4">杩樻病鏈夋垚灏?/h3>
                <p className="text-gray-600 mb-8">鐐瑰嚮銆屾坊鍔犳柊鎴愬氨銆嶆寜閽紑濮嬭褰曚綘鐨勬父鎴忔垚灏?/p>
                <button className="btn-primary" onClick={() => setAddOpen(true)}>
                  <Plus className="w-5 h-5 inline mr-2" />
                  娣诲姞鏂版垚灏?
                </button>
                ...
                {filtered.map((achievement) => (
                  ...
                  >
                    <div className="p-6 bg-white rounded-lg shadow-lg border border-gray-100 h-full">
                      ...
                      <div className="text-sm text-gray-500">鑾峰緱鏃堕棿: {formatDateISO(achievement.date)}</div>
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold text-center mb-12 text-gray-800">鎴愬氨鏃堕棿绾?/h2>
      ...
      <Modal open={addOpen} onClose={() => setAddOpen(false)} title="娣诲姞鏂版垚灏?>
        <AchievementForm
          values={addForm}
      ...
      <Modal open={!!editItem} onClose={() => setEditItem(null)} title="缂栬緫鎴愬氨">
        <AchievementForm
          values={editForm}
      ...
      >
        {detailItem ? (
          <div className="space-y-6">
            ...
            <div>
              <h4 className="font-semibold text-gray-800 mb-2">鎴愬氨鎻忚堪</h4>
              <p className="text-gray-600">{detailItem.description}</p>
            </div>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-gray-600">鑾峰緱鏃ユ湡:</span>
                <span className="font-medium">{formatDateISO(detailItem.date)}</span>
              </div>
            </div>
            {detailItem.screenshot ? (
              <div>
                <h4 className="font-semibold text-gray-800 mb-2">鎴愬氨鎴浘</h4>
                {/* eslint-disable-next-line @next/next/no-img-element */}
