...
function AchievementForm({
  ...
  return (
    <form className="space-y-6" onSubmit={onSubmit}>
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">解锁日期</label>
        <input
          required
          type="date"
          className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">图标样式</label>
        <select
          required
          className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          value={values.icon}
      ...
      {showScreenshotField && (
        <div>
          ...
          <label className="upload-area block cursor-pointer">
            ...
            <p className="text-gray-600 mb-2 text-center">点击上传图片，仅支持JPG/PNG格式</p>
          </label>
          {values.screenshot ? (
...
export function AchievementsClient() {
  ...
  return (
    <>
      ...
      <section className="py-16 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="max-w-6xl mx-auto mb-12 grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">筛选游戏</label>
              <select
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                value={gameFilter}
          ...
          <div className="max-w-6xl mx-auto">
            {filtered.length === 0 ? (
              <div className="text-center py-16">
                ...
                <h3 className="text-2xl font-bold text-gray-700 mb-4">暂无成就数据</h3>
                <p className="text-gray-600 mb-8">快去添加第一条游戏成就记录吧</p>
                <button className="btn-primary" onClick={() => setAddOpen(true)}>
                  <Plus className="w-5 h-5 inline mr-2" />
                  添加成就
                </button>
                ...
                {filtered.map((achievement) => (
                  ...
                  >
                    <div className="p-6 bg-white rounded-lg shadow-lg border border-gray-100 h-full">
                      ...
                      <div className="text-sm text-gray-500">解锁日期：{formatDateISO(achievement.date)}</div>
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold text-center mb-12 text-gray-800">成就解锁时间线</h2>
