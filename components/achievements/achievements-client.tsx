...
function AchievementForm({
  ...
  return (
    <form className="space-y-6" onSubmit={onSubmit}>
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">所属游戏</label>
        <select
          required
          className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          value={values.gameId}
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">获得日期</label>
        <input
          required
          type="date"
          className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">成就图标</label>
        <select
          required
          className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          value={values.icon}
      ...
      {showScreenshotField && (
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">成就截图</label>
          <label className="upload-area block cursor-pointer">
            ...
            <p className="text-gray-600 mb-2 text-center">点击上传截图（JPG/PNG）</p>
          </label>
          {values.screenshot ? (
...
export function AchievementsClient() {
  ...
  const onDelete = (id: number | string) => {
    if (typeof window !== 'undefined' && !window.confirm("确定要删除这个成就吗？此操作不可撤销。")) return;
  ...
  return (
    <>
          ...
          <div className="max-w-6xl mx-auto">
            {filtered.length === 0 ? (
              <div className="text-center py-16">
                ...
                <h3 className="text-2xl font-bold text-gray-700 mb-4">还没有成就</h3>
                <p className="text-gray-600 mb-8">点击「添加新成就」按钮开始记录你的游戏成就</p>
                <button className="btn-primary" onClick={() => setAddOpen(true)}>
                  <Plus className="w-5 h-5 inline mr-2" />
                  添加新成就
                </button>
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold text-center mb-12 text-gray-800">成就时间线</h2>
