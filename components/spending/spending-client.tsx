...
function buildRechargeLinks(): LinkOption[] {
  const games = getGames()
    ...
    .sort((a, b) => (a.name || "").localeCompare(b.name || "", "zh-CN"));
  return [
    { value: RECHARGE_OTHER, label: "其他 / 账号充值", game: DEFAULT_GAME_LABEL },
    ...games.map((g) => ({
      value: String(g.id),
      label: g.name || "未命名",
      game: g.name || "未命名",
...
export function SpendingClient() {
  ...
  const onAddSubmit = (e: React.FormEvent) => {
    ...
    if (!persist([...items, next], "消费记录添加成功")) return;
    setAddForm({
      ...defaultForm(),
  ...
  const onEditSubmit = (e: React.FormEvent) => {
    ...
    if (!persist(next, "消费记录修改成功")) return;
    setEditing(null);
  };
      ...
      <section className="py-10 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-5xl mx-auto">
            ...
            />
            <StatCard title="平均单笔消费" value={fmtMoney(stats.avg)} />
...
function SpendingFormFields({
  ...
  const linkHelp =
    form.recordType === "purchase"
      ? "购买游戏：从愿望单选择要买的游戏，记录会展示在愿望单页面。"
      : "账号充值：从游戏库选择充值对应的游戏，记录会展示在该游戏详情页。";
    ...
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div>
          ...
          >
            ...
            <option value="recharge">账号充值</option>
          </select>
        </div>
        <div>
      ...
      <div>
        ...
        <textarea
          rows={2}
          className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          placeholder="例如：预购豪华版、DLC、游戏内购、Steam礼包充值……"
          value={form.note}
